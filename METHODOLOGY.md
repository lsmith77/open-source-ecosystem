...
### Regulatory integration and emerging conventions (Phase 12)

53. EU Digital Strategy — NIS2 Directive — https://digital-strategy.ec.europa.eu/en/policies/nis2-directive
54. EU Digital Strategy — Cyber Resilience Act open source — https://digital-strategy.ec.europa.eu/en/policies/cra-open-source
...
23. **Credit registry operator spectrum and project sign-off emphasis (human direction).** The human requested clarification of who runs credit registries, noting that the proposal had left this ambiguous, and outlined three operator types: the project itself, a generic third-party platform (e.g., GitHub Sponsors), and a SaaS provider. The human additionally directed that the text stress project sign-off as the ideal rather than treating simpler options as equally valid — and that credit systems should recognize the full spectrum of contributions, not just money and code. The AI added a "Who Operates a Credit Registry?" subsection to Improvement 3 with an operator/example/sign-off table, revised the closing paragraph to establish project sign-off as the goal, and updated the intro paragraph to name contributions that money/code-only registries miss (testing, UX design, documentation, community organization, event work, issue triage). The same framing was applied to the Registry Operator entry in GLOSSARY.md.

24. **Privacy/security risk re-added verbatim by human.** The human identified that risk S3 (Privacy and security exposure in usage declarations) had been removed from a previous revision and provided the full risk text verbatim. The AI re-added it to the Security/Trust section of RISK_ANALYSIS.md and updated the summary matrix. No editorial interpretation was required.

25. **CRA and NIS2 integrated on human direction.** The human asked how the proposal relates to the NIS2 Directive and the Cyber Resilience Act's open-source provisions, then directed the AI to integrate both into the proposal and pitch. The AI fetched both directive pages to understand the specifics before writing. The AI identified two distinct relationships: the CRA creates producer-side obligations (steward concept, SBOM, security policy, vulnerability handling) that map directly onto the `supplyChain` fields; NIS2 creates user-side obligations (supply chain risk management for critical-sector organizations) that the same references and usage declarations support. The AI updated the Legislative Models section of PROPOSAL.md (added detailed CRA and NIS2 paragraphs), added a CRA/NIS2 bullet to Improvement 2's "What This Enables," added targeted bullets to four PITCH.md sections (OSS Project/Maintainer, Vendor/Service Provider, Procurement Office, Policy Maker/Legislator), and added risk P5 (CRA guidance diverges from supply chain model) to RISK_ANALYSIS.md with a Low-Medium/Medium assessment.

26. **CRA Steward Declaration added as deferred improvement on human direction.** The human asked whether steward disclosure should be an extension to publiccode.yml. The AI gave a structured analysis of arguments for (closes the "who is legally responsible" gap, follows the pointer pattern, complements credit registries) and against (CRA guidance not yet settled, it is a legal claim not just metadata, design principle tension around project control). The AI recommended adding it as a deferred improvement pending regulatory guidance. The human directed the AI to proceed. The AI added Improvement 6 (CRA Steward Declaration) at the end of PROPOSAL.md, clearly marked as deferred, with a `maintenance.steward` schema sketch using a pointer-not-claim design (URL to steward's own declaration, stable legal identifier field). The deferred section was added to the ToC and cross-referenced from the CRA Legislative Models paragraph.

27. **`securityPolicy` clarified as format-agnostic; RFC 9116 introduced.** The human noted that SECURITY.md is not a proper standard and queried its status in relation to the proposal. The AI explained that SECURITY.md is a de facto convention popularized by GitHub with no formal specification, while RFC 9116 defines `security.txt` as a proper IETF standard for machine-parseable vulnerability disclosure at `/.well-known/security.txt`. The AI updated the `securityPolicy` schema comment to list multiple valid URL targets (GitHub SECURITY.md, RFC 9116 security.txt endpoint, dedicated security page), added a Design Rationale bullet explaining the distinction and why the field is intentionally format-agnostic, and corrected the "SECURITY.md-style" reference in RISK_ANALYSIS.md risk P5 to accurately reference both conventions. RFC 9116 was added to the METHODOLOGY.md references and to GLOSSARY.md's new Repository-Level Policy File Conventions section.

28. **Emerging repository-level policy file conventions noted by human.** The human provided two URLs (sustainability.md and ACCESSIBILITY.md by mgifford) as emerging conventions analogous to SECURITY.md. The AI fetched both to understand their scope: SUSTAINABILITY.md covers environmental impact across product, code, and operations following W3C Web Sustainability Guidelines; ACCESSIBILITY.md covers WCAG conformance, known gaps, contributor expectations, and CI/CD integration for accessibility checks. The AI added a Design Rationale bullet in Improvement 2 noting the broader pattern of named policy files in repository roots, explicitly scoping them out of `supplyChain` (which covers supply chain and compliance artifacts) and identifying the future `supports` key as the natural extension point as those conventions mature. Both were added to GLOSSARY.md under a new "Repository-Level Policy File Conventions" section alongside SECURITY.md and RFC 9116.
