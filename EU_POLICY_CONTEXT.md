...
| [Public Procurement Act](#public-procurement-act)                                                                                               | Proposal delayed; Q2 2026 target missed, expected Q3 2026                                                                                                                                                                                                                          | Non-price criteria; "Made in Europe"; SME access                                                            | Metadata infrastructure makes non-price criteria measurable                                           |
...
The deferred [Improvement 8: CRA Steward Declaration](PROPOSAL.md#improvement-8-cra-steward-declaration-deferred) would add an explicit `crasteward` field to publiccode.yml, making steward identity machine-discoverable for regulators and deploying organizations.
...
The toolbox names the _goals_ — supplier diversity, reduced dependency on any single high-risk supplier — but, like the rest of the regulatory landscape, provides no mechanism to measure them for open source software:

- **Multi-vendor strategies:** For OSS, the relevant "supplier diversity" is upstream maintainer diversity. The [credit registry](PROPOSAL.md#improvement-3-vendor-contribution-credit-system-discovery) reveals whether a project has multiple independent organizations contributing and offering support — the structural test for whether a multi-vendor strategy is even possible for a given component.
...
> **Scope note:** This is the Commission's _internal_ strategy — how the Commission produces and consumes open source — distinct from the [Interoperable Europe Act](#interoperable-europe-act), which places reuse obligations on _all_ EU public bodies. But it established the template (publiccode, OSPO, contribute-upstream) the Act later generalized, making it the conceptual "why" behind much of this proposal.
...
| ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Share**           | publiccode.yml + [faceted classification](PROPOSAL.md#improvement-1-faceted-classification-replacing-flat-categories) make shared code discoverable, not just published                                           |
| **Contribute**      | [Credit registries](PROPOSAL.md#improvement-3-vendor-contribution-credit-system-discovery) make upstream contribution verifiable rather than asserted                                                             |
...
**Key insight:** The EU Open Source Strategy is the principled "why"; this proposal is the measurement "how." Its commitments to _share_, _contribute_, _secure_, and _stay in control_ are behaviors that remain unverifiable without standardized discovery, contribution, and supply chain metadata. Registries turn the strategy's principles into auditable signals.
...
**Status:** Pre-legislative. Commission proposal expected Q2 2026 — not published; Q2 2026 target missed. Now expected Q3 2026. Public consultation closed January 2026.
...
- **Same objective, different layer.** The DMA attacks lock-in at the _gatekeeper-platform_ layer through interoperability and portability mandates; this proposal attacks it at the _software-procurement_ layer. Both serve the vendor-lock-in reduction goal made an inter-institutional baseline by the [Council Conclusions](#council-conclusions-on-european-competitiveness-in-the-digital-decade).
...
- **The proposal reaches where the DMA cannot.** The DMA binds only a small set of _designated_ gatekeepers, ex ante — nothing for a procurement office choosing software in the long tail. Usage and credit registries surface whether a given option has independent, multi-vendor support for software entirely outside the DMA's scope.
...
- **Tier 4 is a software-supply-chain claim.** "Full transparency and control over the software supply chain with no third-country interference" is exactly the property the [`supplyChain` references](PROPOSAL.md#improvement-2-supply-chain-references) (SBOM, security policy, Scorecard) and the [credit registry](PROPOSAL.md#improvement-3-vendor-contribution-credit-system-discovery) make machine-verifiable. A conformity assessment against the higher CADA tiers needs the upstream-maintenance and supply-chain evidence registries supply — the same day-1/day-2 sovereignty test in the [Carrez framing](#the-european-preference-tension-with-open-source).
...
- **Open source for resilience must be discoverable to be chosen.** CADA's promotion of open source as a resilience measure presupposes that resilient, multi-vendor-supported alternatives can be found and compared — the function of [faceted classification](PROPOSAL.md#improvement-1-faceted-classification-replacing-flat-categories) and usage/credit registries.

#### The operational machinery: Cloud Sovereignty Framework (CSF/SEAL)

CADA's "four sovereignty tiers" are not abstract. They codify, at Union level, the **[Cloud Sovereignty Framework (CSF) v1.2.1](https://commission.europa.eu/document/download/09579818-64a6-4dd5-9577-446ab6219113_en)** that DG DIGIT published in October 2025 and first applied in its own Cloud III procurement (April 2026). The CSF assesses providers across **8 Sovereignty Objectives (SOV-1…SOV-8)** and rates each on a **5-level Sovereignty Effectiveness Assurance Level scale (SEAL-0…SEAL-4)**, combined via a per-objective **Minimum Assurance Level** (a rejection threshold) and a weighted **Sovereignty Score** (an award criterion). SEAL is assessed _per objective_, not as a single holistic provider tier — a nuance the "four tiers" shorthand obscures.

This matters because the CSF is the concrete evidence structure this proposal's registries feed. The objectives map onto the metadata components almost line-for-line:

| CSF Sovereignty Objective (weight)                                                          | Proposal component supplying the evidence                                                                                                         |
| ------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| **SOV-5 Supply Chain** (20%, highest)                                                       | [`supplyChain` references](PROPOSAL.md#improvement-2-supply-chain-references) — SBOM, OpenSSF Scorecard, sub-supplier visibility                  |
| **SOV-6 Technology** (15%) — open software, audit/modification rights, non-proprietary APIs | Open-source posture + [Platform Lock-in Declaration](PROPOSAL.md#improvement-9-platform-lock-in-declaration-proposed)                                      |
| **SOV-1 Strategic** (15%) — operational continuity if vendors disconnect                    | [Credit registry](PROPOSAL.md#improvement-3-vendor-contribution-credit-system-discovery) — the day-1/day-2 [Carrez test](#the-european-preference-tension-with-open-source) |
| **SOV-4 Operational** (15%) — EU operator capability and talent                             | Credit registry — who can actually maintain and operate the software                                                                              |
| **SOV-7 Security & Compliance** (10%)                                                       | `supplyChain` references — GDPR/NIS2/DORA evidence                                                                                                |

**The decisive operational hook is the assessment registry.** The CSF Implementation Guidance identifies the **per-tender self-assessment burden** as the main adoption blocker, calling for standardised questions, harmonised evidence requirements, and shared public-good evaluation infrastructure that contracting authorities can reuse. That is precisely the [Assessment Registry](PROPOSAL.md#assessment-registry-api-proposed): a SOV/SEAL assessment published **once** and reused by every contracting authority, rather than re-run per tender. The CSF is, in registry terms, simply another named assessment framework alongside national Sovereignty Checks.

#### CADA Chapter V (Articles 41–44) — what each article does, and where the metadata fits

The Commission's June 2026 feedback request focuses on Chapter V. In their **adopted** form these articles set direction without enforceable obligations; each, however, presupposes machine-readable evidence this proposal supplies:

- **Article 41 (open source first).** Frames an _encouragement_ to use and reuse open standards and open source components, with broad "duly justified objective criteria" derogations — the same soft-law verb as France's 2016 Loi Lemaire and Italy's CAD Art. 68. Stakeholders (CNLL, the July 2025 European software publishers' letter) press for a **comply-or-explain** mechanism with a documented, public, auditable derogation assessment. That mechanism only works on standardised evidence: [faceted classification](PROPOSAL.md#improvement-1-faceted-classification-replacing-flat-categories) makes the available OSS alternatives discoverable, and the [`.well-known/publiccode-usage.json` usage-declaration layer](PROPOSAL.md#organization-level-usage-declarations) makes both the choice and its justification auditable rather than narrative.
- **Article 42 (sharing and reuse).** Requires reusable public-sector software to be made available "using a catalogue or repository **connected to, and made accessible through**, the EU OSS Catalogue (Art. 43)." This is a _federation_ requirement, not just an onboarding format — exactly what the [Registry Discovery Standard](PROPOSAL.md#registry-discovery-standard-rough-outline) (`/.well-known/publiccode-registry.json`) and federated-catalog design enable, letting national catalogues (France's SILL — _Socle Interministériel de Logiciels Libres_ — Germany's openCode, Italy's Developers Italia) connect without a central gatekeeper.
- **Article 43 (EU OSS Catalogue).** Codifies the existing Interoperable Europe Portal catalogue. To become a _procurement aid_ rather than a reference list, each entry needs the structured "Buyer's Guide" evidence stakeholders call for: identifiable EU publisher providing support ([credit registry](PROPOSAL.md#improvement-3-vendor-contribution-credit-system-discovery)), peer-administration deployments ([usage registry](PROPOSAL.md#usage-registry-api)), licence, and SBOM ([`supplyChain` references](PROPOSAL.md#improvement-2-supply-chain-references)). The proposal's registry triad is independently the catalogue infrastructure this requires; proposed inclusion criteria (CSF/SEAL vetting + SBOM availability + active maintenance + identifiable European publisher) are all machine-readable from these fields.
- **Article 44 (OSPO Network).** Institutionalises the existing public-sector OSPO network. Stakeholders argue OSPOs should treat **contribution measurement** as a strategic stewardship indicator (not only consumption metrics) and steer procurement via an **allotment doctrine** that contracts the upstream publisher's subscription separately from the integrator's services — both of which need visibility into who the actual upstream maintainer is versus the reselling integrator, which is what the [credit registry](PROPOSAL.md#improvement-3-vendor-contribution-credit-system-discovery) surfaces.

**Definitional safeguard.** The Strategy anchors the legal definition of OSS on the [OSI definition](https://opensource.org/osd) (with EUPL, GPL, Apache-2.0, MIT and peers as qualifying licences), extendable to AI via the [Open Source AI Definition (OSAID 1.0)](https://opensource.org/ai) and the G7 "open weights / open data / open code" vocabulary. This proposal's sovereignty and contribution signals must stay **orthogonal** to that licence-based definition: they score and annotate the _producers_ of software, they do not redefine what _qualifies_ as open source. Keeping the two separate (consistent with [Design Principle 10](PROPOSAL.md#design-principles) and the [Carrez framing](#the-european-preference-tension-with-open-source)) guards against a "public governance" qualifier being smuggled into the definition through procurement-design rules — a risk the ecosystem has repeatedly flagged.
