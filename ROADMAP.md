...
| **BFH / CHopen**                                | Operate ossdirectory.com—a natural crawler/catalog that benefits immediately from standardized metadata. Academic credibility.                                            | Existing relationship through Schleswig-Holstein |
| **[Zentrum SDS](https://zentrumsds.ch/) (Souveräne Digitale Schweiz)** | Swiss digital-sovereignty center initiated by BFH, with 40+ public/private partners (cantons Bern, Basel-Stadt, Solothurn; City of Zürich; Swisscom, Adfinis, Puzzle ITC, VSHN, and others) and a cooperation with ZenDiS on openDesk/openCode. Planning a Forgejo-based **opencode.ch** forge for Swiss public-sector projects—a controlled platform where authoring, validation, badges, and `.well-known` usage publication can be built in at the platform level and (Forgejo being open source) upstreamed. | Initiated by BFH (project's coalition) |
...
2. **Update validators and build a classification wizard** in a publiccode.yml editor — the schema validator must support the new `classification` field, and the wizard reduces the complexity risk (A6) by guiding maintainers through facet selection. See [TOOLING.md](TOOLING.md) for the editor/validator tooling and gaps.
...
7. **Prototype an internal scan-and-publish tool** that organizations can run to inventory deployed software and generate a `.well-known/publiccode-usage.json` or bulk-import into a registry. See [TOOLING.md](TOOLING.md) for the tooling landscape and gaps.
...
7. **Engage legislators using the Swiss EMBAG as a model.** Document EMBAG's outcomes and package a "legislative toolkit" — model clauses that reference publiccode.yml metadata for procurement compliance, supply chain transparency, and vendor evaluation. Coordinate with FSFE's Public Money? Public Code! campaign to connect legislation advocacy with the metadata infrastructure that makes compliance practical. The primary EU-level legislative hooks are the Interoperable Europe Act (already in force), the forthcoming EU Public Procurement Act (still expected Q2–Q3 2026; not yet tabled as of June 2026), and — newly added on 3 June 2026 — the **European technological sovereignty package**: the Cloud and AI Development Act (now a formal legislative proposal, with a common EU-level procurement framework for public administrations and cloud sovereignty tiers reaching into the software supply chain) and the successor **Open Source Strategy** (whose public-administration adoption goal this infrastructure directly serves). See [EU_POLICY_CONTEXT.md](EU_POLICY_CONTEXT.md) for a structured analysis of each.
8. **Ship a turnkey credit system for OSS projects.** Based on the Phase 3 reusability assessment, package Drupal's `contribution_records` module (or a derivative) as a standalone credit system that any OSS project can deploy — either self-hosted or as a hosted SaaS. Provide integrations for common issue trackers (GitHub, GitLab, Jira) and out-of-the-box Credit Registry API conformance. This lowers the barrier for projects that want vendor credit tracking but lack the resources to build it themselves. See [TOOLING.md](TOOLING.md) for the tooling landscape and gaps.
