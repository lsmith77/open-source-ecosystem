...
- Its [publiccode-action](https://github.com/OpenCatalogi/publiccode-action) auto-generates and maintains the `publiccode.yaml` from repository metadata (name, description) and, on every push to `main`, bumps `softwareVersion`/`releaseDate` and fires a "federalize" event to the OpenCatalogi API to trigger re-indexing. It is an extended version of [Italy's publiccode-action](https://github.com/italia/publiccode-parser-action).
