...
The success of this proposal depends on broad stakeholder engagement and a clear understanding of how the standard is governed. Under the publiccode.yml [charter](https://github.com/publiccodeyml/publiccode.yml/blob/main/governance/charter.md) and [proposal-and-voting procedure](https://github.com/publiccodeyml/publiccode.yml/blob/main/governance/procedure-proposing-changes-and-voting.md), anyone may open a PR, but voting is restricted to public servants on a Steering Committee with one seat per country (currently France, Italy, and the Netherlands); the maintainer merges a change once the Committee has reached consensus. The default threshold is unanimity, and the improvements proposed here are backward-compatible additions and deprecations, so they qualify as minor-version changes rather than triggering the two-year breaking-change gate. Engaging the Committee and maintainers early — alongside the broader working group that drafts and pilots the improvements — is therefore essential. The following process is recommended:

1. **Steering Committee and maintainer engagement:** Engage the FR/IT/NL Steering Committee members and the maintainers early. The Committee votes on changes and the maintainers implement them; both need to understand the improvements, and the maintainers need capacity (co-maintenance, tooling) to absorb the scope increase.
...
**Sovereignty-driven framework: European technological sovereignty package (3 June 2026).** The Commission's [tech sovereignty package](https://ec.europa.eu/commission/presscorner/detail/en/ip_26_1187) added two relevant instruments. The **[Cloud and AI Development Act (CADA)](https://digital-strategy.ec.europa.eu/en/policies/cloud-and-ai-development-act)** — now a formal legislative proposal — sorts cloud providers into sovereignty tiers, the highest of which requires control over the software supply chain with no third-country interference, and establishes a common EU-level procurement framework for public administrations. The entity `identifier` fields, `contributionCreditRegistries` (who actually maintains the project), and `supplyChain` references are the evidence base a CADA tier assessment draws on — resolving control and jurisdiction from authoritative sources rather than duplicating them (see [Design Principle 4](#design-principles)). The successor **2026 Open Source Strategy**, whose stated goal is greater use of open source in public administrations, is served directly by the `.well-known/publiccode-usage.json` adoption layer.

For a detailed treatment of each initiative — including the [tech sovereignty package](EU_POLICY_CONTEXT.md#european-technological-sovereignty-package-3-june-2026), the forthcoming Public Procurement Act, and the Interoperable Europe Act already in force — see [EU_POLICY_CONTEXT.md](EU_POLICY_CONTEXT.md).
...
**SDG relevance (policy facet).** Projects may assert relevance to the UN [Sustainable Development Goals](https://sdgs.un.org/goals) via an `sdg:` facet in `tags` (e.g. `sdg:3`, `sdg:7`), using the UN's fixed 17-goal / 169-target vocabulary. Keeping this a facet rather than a core dimension lets SDG awareness spread across open source generally without privileging one body's frame in the base schema. Like all publiccode.yml fields it is a maintainer self-assertion — a broad, often aspirational signal that carries little weight on its own. Verification stays with recognizing bodies such as the [Digital Public Goods Alliance](https://www.digitalpublicgoods.net/), whose index of recognized Digital Public Goods is joined to the project at the catalog layer via its stable identifier (per the lookup-key principle), never written back into the file. The facet widens participation; DPGA recognition remains the gold-standard trust anchor.
...
A contribution credit registry can be operated by the project itself, by a generic third-party platform, or by a SaaS provider — but the operator is an implementation detail, not the locus of authority. **The OSS project governs its own contribution credit.** By listing a registry in `contributionCreditRegistries` in its publiccode.yml, the project both designates which system holds the authoritative credit data and endorses that data as its own: "we consider this data authoritative for our contributors."

**Authority via source control.** Because that pointer lives in the project's own source repository, it carries the same authorial authority as the rest of the file — the same principle that anchors [package repositories](#improvement-6-package-distribution-references) and [sanctioned mirrors](#improvement-7-sanctioned-mirror-declarations). No outside party can endorse a registry on the project's behalf, and the project revokes an endorsement by removing the entry. The endorsement is therefore always authoritative and machine-checkable on its face; what varies between operators is only how deeply the project curates the underlying credit records, not whether the endorsement counts.

| Operator                         | Example                                                                                                                      | Sign-off on underlying records                                                                       |
| -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
...
The endorsement is unconditional, but the depth of the project's review of the underlying records scales with the registry's sophistication — and that spectrum is not flat: active sign-off is the goal, not just one option among equals. A project that points at a GitHub Sponsors page still issues a fully authoritative endorsement, but it is endorsing a third-party platform's data without reviewing the underlying attribution: a low-friction starting point, not the intended end state. The ideal is for the project to take active responsibility for contribution credits: deciding what contribution types count (code, testing, design, organization, documentation, etc.), reviewing whether the data accurately reflects who sustains the project, and defining a process for sign-off. Projects that invest in this — whether through a custom registry, a SaaS platform with maintainer review, or a formal contribution credit policy — produce data that is meaningfully more trustworthy for procurement decisions and fairer to the full range of contributors.
...
**Endorsement, not a copy of the support matrix.** The project-controlled value here is the _endorsement_ — which distributions the project officially supports. The optional `architectures` and `version_constraint` detail duplicates per-release data that package registries (ecosyste.ms, deps.dev) hold authoritatively and that ages between releases; prefer resolving it from the registry via the PURL and keep `package_repositories` scoped to the endorsement itself (see Design Principles 2 and 4).
...
| `software[].certifications[]` | Certifications and audits obtained by the deploying organization for this specific deployment — each entry carries `type` (`accessibility`, `security`, `privacy`, `compliance`), `standard` (e.g., `WCAG 2.1`, `ISO 27001`), optional `level` (e.g., `AA`, `High`), `issued` date, optional `expires` date, `issuer`, and optional `reference` URL. Optional. |
