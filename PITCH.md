...
- **Turn CRA obligations into a competitive advantage** — the [Cyber Resilience Act](https://digital-strategy.ec.europa.eu/en/policies/cra-open-source) introduces the *open-source software steward* role for projects with sustained commercial use. If your project (or an organization backing it) takes on steward responsibilities, the `supplyChain` fields make the required security artifacts — SBOM, vulnerability disclosure policy, security development practices — machine-discoverable to every catalog and procurement office in the ecosystem. Compliance becomes visible proof of project maturity, not just overhead.
...
- **Position as a CRA-recognized steward** — the [Cyber Resilience Act](https://digital-strategy.ec.europa.eu/en/policies/cra-open-source) formalizes what many vendors already do: provide sustained support for an OSS product used in commercial contexts. Credit registries provide the verifiable contribution track record that demonstrates "sustained support." The `supplyChain` fields provide the security compliance evidence. Vendors who establish both before the market settles will be the natural candidates for formal steward roles — and the associated procurement preference that will follow.
